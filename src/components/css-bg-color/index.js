...
class PGcssBgColor extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <div className='my-4'>
          <p className='text-sm'>{__("This css property is depricated, please use", "combo-blocks")} <strong>{__("Background Color", "combo-blocks")}</strong> {__("instead", "combo-blocks")}</p>
          <div className='relative h-10' style={placeholderStyle}>
            <div className='absolute w-full  h-full top-0 left-0 text-center' style={btnStyle} onClick={this.handleToggleClick}>
              <span className='w-full text-center left-0 top-1/2 -translate-y-1/2	 absolute'>{(val == undefined) ? __('Set Color', "combo-blocks") : val}</span>
            </div>
          </div>
        </div>
