...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var front = attributes.front;
		var back = attributes.back;
		...
		var frontSelector = blockClass + " .pg-flip-box-front";
		var backSelector = blockClass + " .pg-flip-box-back";
		useEffect(() => {
		...
		useEffect(() => {
			...
			blockCssObj[frontSelector] = front;
			blockCssObj[backSelector] = back;
		...
		function onPickBlockPatterns(content, action) {
			...
			if (action == "applyStyle") {
				...
				var frontX = attributes.front;
				var backX = attributes.back;
				...
				if (frontX != undefined) {
					var frontY = { ...frontX, options: front.options };
					setAttributes({ front: frontY });
					blockCssObj[frontSelector] = frontY;
				}
				if (backX != undefined) {
					var backY = { ...backX, options: back.options };
					setAttributes({ back: backY });
					blockCssObj[backSelector] = backY;
				}
				var blockCssRules = myStore.getBlockCssRules(blockCssObj);
				var items = blockCssRules;
				setAttributes({ blockCssY: { items: items } });
		...
		function onChangeStylefront(sudoScource, newVal, attr) {
			var path = [sudoScource, attr, breakPointX];
			let obj = Object.assign({}, front);
			const object = myStore.updatePropertyDeep(obj, path, newVal);
			setAttributes({ front: object });
			var elementSelector = myStore.getElementSelector(
				sudoScource,
				frontSelector
			);
		...
		function onRemoveStylefront(sudoScource, key) {
			let obj = { ...front };
			var object = myStore.deletePropertyDeep(obj, [
				sudoScource,
				key,
		...
		function onResetfront(sudoSources) {
			let obj = Object.assign({}, front);
			Object.entries(sudoSources).map((args) => {
				var sudoScource = args[0];
				if (obj[sudoScource] == undefined) {
			...
			setAttributes({ front: obj });
		}
		function onAddStylefront(sudoScource, key) {
			var path = [sudoScource, key, breakPointX];
			let obj = Object.assign({}, front);
			const object = myStore.addPropertyDeep(obj, path, "");
			setAttributes({ front: object });
		}
		function onBulkAddfront(sudoScource, cssObj) {
			let obj = Object.assign({}, front);
			obj[sudoScource] = cssObj;
			setAttributes({ front: obj });
			var selector = myStore.getElementSelector(sudoScource, frontSelector);
			var stylesObj = {};
			Object.entries(cssObj).map((args) => {
				var attr = args[0];
				var cssPropty = myStore.cssAttrParse(attr);
		...
		function onChangeStyleback(sudoScource, newVal, attr) {
			var path = [sudoScource, attr, breakPointX];
			let obj = Object.assign({}, back);
			const object = myStore.updatePropertyDeep(obj, path, newVal);
			setAttributes({ back: object });
			var elementSelector = myStore.getElementSelector(
				sudoScource,
				backSelector
			);
		...
		function onRemoveStyleback(sudoScource, key) {
			let obj = { ...back };
			var object = myStore.deletePropertyDeep(obj, [
				sudoScource,
				key,
		...
		function onResetback(sudoSources) {
			let obj = Object.assign({}, back);
			Object.entries(sudoSources).map((args) => {
				var sudoScource = args[0];
				if (obj[sudoScource] == undefined) {
			...
			setAttributes({ back: obj });
		}
		function onAddStyleback(sudoScource, key) {
			var path = [sudoScource, key, breakPointX];
			let obj = Object.assign({}, back);
			const object = myStore.addPropertyDeep(obj, path, "");
			setAttributes({ back: object });
		}
		function onBulkAddback(sudoScource, cssObj) {
			let obj = Object.assign({}, back);
			obj[sudoScource] = cssObj;
			setAttributes({ back: obj });
			var selector = myStore.getElementSelector(sudoScource, backSelector);
			var stylesObj = {};
			Object.entries(cssObj).map((args) => {
				var attr = args[0];
				var cssPropty = myStore.cssAttrParse(attr);
									...
									onClick={(ev) => {
										replaceInnerBlocks(
											...
											createBlocksFromInnerBlocksTemplate([
												["combo-blocks/flip-box-front", { blockId: "XYZ123" }],
												["combo-blocks/flip-box-back", { blockId: "XYZ123" }],
											]),
											true
										);
									}}>
