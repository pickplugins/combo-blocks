...
import queryPramsArgs from "./queryprams";
...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var editMode = attributes.editMode == undefined ? false : attributes.editMode;
		var liveMode = attributes.liveMode == undefined ? false : attributes.liveMode;
		...
		var queryPrams = applyFilters("PGpostQueryPramsArgs", queryPramsArgs);
		...
		let queryPresets = applyFilters("PGqueryPresets", queryPresetsBasic);
		var parentQueryArgs =
		...
		var [layoutData, setLayoutData] = useState({ source: "" });
		...
		function fetchLayouts() {
			...
			if (layoutData.source.length == 0) return;
		...
		const postTypesData = useSelect(
			(select) => select(coreStore).getPostTypes({ per_page: -1, public: false }),
			[]
		);
		postTypesData !== null &&
			postTypesData.map((x) => {
					...
					<div className="pg-setting-input-text">
						<div className="p-3">
							<ToggleControl
								...
								help={editMode ? "Edit Mode Enabled(May slowdown editor.)" : "Edit Mode Disabled."}
						...
						<div className="p-3">
							<ToggleControl
								label={__("Live Mode?", "combo-blocks")}
								help={liveMode ? "Edit Mode Enabled" : "Edit Mode Disabled."}
								checked={liveMode ? true : false}
								onChange={(e) => {
									setAttributes({ liveMode: liveMode ? false : true });
								}}
							/>
						</div>
