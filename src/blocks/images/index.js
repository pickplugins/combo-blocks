...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var sourceList = {
			...
			acfGallery: { label: "ACF Gallery", value: "acfGallery", },
			customField: { label: "Custom Field", value: "customField", },
			imgUrls: { label: "Image URLs", value: "imgUrls", },
			// dropbox: { label: "Dropbox", value: "dropbox", isPro: true },
			...
			<>
				<InspectorControls>
					...
					<div className="pg-setting-input-text">
						<PGtoggle title="Images" initialOpen={true}>
							<PanelRow>
								<label>Source?</label>
								<PGDropdown
							...
							{wrapper?.options?.source == 'imgUrls' && (
								<>
									<div>
										<label className="font-medium text-slate-900 ">{__("Image URLs", "combo-blocks")}</label>
										<TextareaControl
											value={wrapper.options.sourcePrams.content}
											onChange={(newVal) => {
												var sourcePramsX = { ...wrapper.options.sourcePrams, content: newVal };
												var options = { ...wrapper.options, sourcePrams: sourcePramsX };
												setAttributes({ wrapper: { ...wrapper, options: options } });
											}}
										/>
									</div>
									<p>Keep image url in each line.</p>

									<PanelRow>
										<label className="font-medium text-slate-900 ">{__("Max?", "combo-blocks")}</label>
										<InputControl
											value={wrapper.options.sourcePrams.max}
											onChange={(newVal) => {
												var sourcePramsX = { ...wrapper.options.sourcePrams, max: newVal };
												var options = { ...wrapper.options, sourcePrams: sourcePramsX };
												setAttributes({ wrapper: { ...wrapper, options: options } });
											}}
										/>
							...
							{wrapper?.options?.source == 'acfGallery' && (
								<>
									<PanelRow>
							...
							{wrapper?.options?.source == 'customField' && (
								<>



									<PanelRow>
										<label className="font-medium text-slate-900 ">{__("Meta Type", "combo-blocks")}</label>
										<SelectControl
											label=""
											value={wrapper.options.sourcePrams.type}
											options={[
												{ label: __("Choose", "combo-blocks"), value: "" },
												{ label: "Comma Separate Ids", value: "commaSeparateIds" },
												{ label: "Array Ids", value: "arrayIds" },

											]}
											onChange={(newVal) => {
												var sourcePramsX = { ...wrapper.options.sourcePrams, type: newVal };
												var options = { ...wrapper.options, sourcePrams: sourcePramsX };
												setAttributes({ wrapper: { ...wrapper, options: options } });
											}}
										/>
															...
															return (
																<div
																	...
																	<span
																		...
																		onClick={(ev) => {
																			var galleryItemsX = [...galleryItems];
																			galleryItemsX[index].categories.splice(
																				j,
																				1
																			);
																			setAttributes({
