...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var overlay = attributes.overlay;
		...
		var overlaySelector = blockClass + " .overlay";
		var summarySelector = blockClass + " .summary";
		...
		useEffect(() => {
			...
			blockCssObj[overlaySelector] = overlay;
		...
		function onChangeStyleOverlay(sudoScource, newVal, attr) {
			var path = [sudoScource, attr, breakPointX];
			let obj = Object.assign({}, overlay);
			const object = myStore.updatePropertyDeep(obj, path, newVal);
			setAttributes({ overlay: object });
			var elementSelector = myStore.getElementSelector(
				sudoScource,
				overlaySelector
			);
		...
		function onAddStyleOverlay(sudoScource, key) {
			var path = [sudoScource, key, breakPointX];
			let obj = Object.assign({}, overlay);
			const object = myStore.addPropertyDeep(obj, path, "");
			setAttributes({ overlay: object });
		}
		function onBulkAddOverlay(sudoScource, cssObj) {
			let obj = Object.assign({}, overlay);
			obj[sudoScource] = cssObj;
			setAttributes({ overlay: obj });
			var selector = myStore.getElementSelector(sudoScource, overlaySelector);
			var stylesObj = {};
			Object.entries(cssObj).map((args) => {
				var attr = args[0];
				var cssPropty = myStore.cssAttrParse(attr);
		...
		function onRemoveStyleOverlay(sudoScource, key) {
			let obj = { ...overlay };
			var object = myStore.deletePropertyDeep(obj, [
				sudoScource,
				key,
		...
		function onResetOverlay(sudoScources) {
			let obj = Object.assign({}, overlay);
			Object.entries(sudoScources).map((args) => {
				var sudoScource = args[0];
				if (obj[sudoScource] == undefined) {
				...
				{elements.items.length > 0 && (
					<div {...blockProps}>
						...
						{summary?.options?.type.length > 0 && (
							<div className="summary" onClick={handleOpen}>
								{summary.options.type !== "custom" &&
