...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var itemSelector = blockClass + " .pg-terms-showcase-item ";
		...
		useEffect(() => {
			...
			nthItemStyle.map((x, i) => {
				var selector = `${blockClass} .pg-terms-showcase-item:nth-child(${i + 1
					})`;
		...
		const ALLOWED_BLOCKS = ["combo-blocks/terms-query"];
		const MY_TEMPLATE = [
	...
	save: function (props) {
		...
		const blockProps = useBlockProps.save({
			className: ` ${blockId}  pg-terms-showcase-item `,
		});
