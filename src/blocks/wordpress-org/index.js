...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var objectX = attributes.objectX;
		...
		var objectTypes = {
			plugin: { label: __("Plugins", "combo-blocks"), value: "plugin" },
			theme: { label: __("Themes", "combo-blocks"), value: "theme" },
		};
		...
		useEffect(() => {
			...
			debounce = setTimeout(() => {
				apiFetch({
					path: "/combo-blocks/v2/wordpress_org_data",
					method: "POST",
					data: postData,
				}).then((res) => {
					seloading(false);
					if (res.data == undefined) {
						...
						setAttributes({ objectX: { ...objectX, options: optionsX } });
					}
				});
			}, 2000);
		}, [object.options.slug]);
		...
		const MY_TEMPLATE = [["combo-blocks/wordpress-org-item", {}]];
		...
		const addChild = () => {
			...
			const slide = createBlock("combo-blocks/wordpress-org-item");
