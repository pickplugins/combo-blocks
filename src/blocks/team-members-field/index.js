...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var teamMemberData = context["teamMemberData"];
		var loopIndex = context["loopIndex"];
		var socialLinks = context["socialLinks"];
		...
		var userFieldsBasic = {
			...
			website: {
				label: __("Website", "combo-blocks"),
				value: "website",
			},
			phone: {
				label: __("Phone", "combo-blocks"),
				value: "phone",
			},
			role: {
				label: __("Role", "combo-blocks"),
				value: "role",
			},
			categories: {
				label: __("Categories", "combo-blocks"),
				value: "categories",
			},
			socialLinks: {
				label: __("Social Links", "combo-blocks"),
				value: "socialLinks",
			},
		...
		// const fieldItemSelector = blockClass + " .fieldVal > .item";
		// const fieldItemIconSelector = blockClass + " .fieldVal > .item > .icon";
						...
						<CustomTagWrapper {...blockProps}>
							...
							{field.options.linkTo.length == 0 && (
								<>
									...
									{metaKey === "title" && (
										<span className="fieldVal">{teamMemberData[metaKey]}</span>
									)}
									{metaKey === "role" && (
										<span className="fieldVal">{teamMemberData[metaKey]}</span>
									)}
									{metaKey === "description" && (
										<span
											...
											dangerouslySetInnerHTML={{
												__html: teamMemberData["description"],
											}}></span>
									)}
									{metaKey === "website" && (
										<a className="fieldVal" href={teamMemberData["website"]}>
											{teamMemberData["website"]}
										</a>
									)}
									{metaKey === "phone" && (
										<a className="fieldVal" href={teamMemberData["phone"]}>
											{teamMemberData[metaKey]}
										</a>
									)}
									{metaKey === "custom" && (
									...
									{metaKey == "socialLinks" && (
										<span className="fieldVal social-links">
											{teamMemberData !== undefined &&
												teamMemberData.socialLinks.map((item, index) => {
													var socialLinkConfig = socialLinks.find(
														(link) => link.id === item.id
													);
													var socialIconHtml = "";
													if (item.icon != undefined) {
														socialIconHtml = `<span class="${socialLinkConfig?.siteIcon?.iconSrc}"></span>`;
													}
													return (
														<a
															key={index}
									...
									{metaKey == "categories" && (
										<span className="fieldVal categories">
											{teamMemberData !== undefined &&
												teamMemberData.categories.map((item, index) => {
													return (
														<>
															<span key={index} className={`category-${index}`}>
																{item}
															</span>
															{teamMemberData.categories.length > index + 1 && (
																<>
																	{field.options.separator.length > 0 && (
