...
registerBlockType(metadata, {
		...
		from: [
			{
				...
				transform: (attributes) => {
					return createBlock("combo-blocks/post-author-fields", {
						wrapper: {
							options: {
	...
	edit: function (props) {
		...
		var userFieldsBasic = {
			...
			registered: { label: "Registered", value: "registered" },
			display_name: { label: "Display name", value: "display_name" },
		...
		// function findUserFieldsById(key) {
		// 	return userFields.find((item) => item.key === key);
		// }
		// const metaKeyX = findUserFieldsById(metaKey);
		var linkToArgsBasic = {
		...
		var parentData =
			context["combo-blocks/postElement"] == undefined
				? null
				: context["combo-blocks/postElement"];
		useEffect(() => {
			if (parentData != null) {
				// setAttributes({ object: parentObject });
				setLoading(true);
				if (parentData.data !== undefined) {
					setpostAuthorData(parentData.data);
					setLoading(false);
				}
			}
		}, [parentData]);
