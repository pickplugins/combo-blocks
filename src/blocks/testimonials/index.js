...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var formattedPrompt = "Respond only with json array useing following format {id: '',name: 'person name',url: 'Profile image url',role: 'person role',message: 'message',website: 'website url',phone: '',socialLinks: [],skills: [],categories: [],} and no other text. Do not include any explanations, introductions, or concluding remarks.";
		var [breakPointX, setBreakPointX] = useState(myStore.getBreakPoint());
		var testimonial = attributes.testimonial;
		...
		var sourceList = {
			...
			// trustpilot: { label: "Trustpilot", value: 'trustpilot', },
			// yelp: { label: "Yelp", value: 'yelp', },
		...
		var googleReviewsArgs = {
			input: { label: "input", value: 'input' },
			inputtype: { label: "inputtype", value: 'inputtype' },
			fields: { label: "fields", value: 'fields' },
			placeid: { label: "placeid", value: 'placeid' },
		}
		var trustpilotReviewsArgs = {
			page: { label: "page", value: 'page' },
			perPage: { label: "perPage", value: 'perPage' },
			stars: { label: "stars", value: 'stars' },
			language: { label: "language", value: 'language' },
			includeReported: { label: "includeReported", value: 'includeReported' },
		}
		...
		const MY_TEMPLATE = [["combo-blocks/testimonials-field", {}]];
		...
		const TEMPLATE = [["combo-blocks/testimonials-field", {}]];
		...
		useEffect(() => {
			...
			if (parentBlock != null) {
				...
				if (parentBlock.name == "combo-blocks/testimonial-showcase") {
					className = " pg-testimonial-showcase-item ";
				}
		...
		const pasteData = () => {
			navigator.clipboard
				...
				.then((text) => {
					...
					setAttributes({ testimonial: parsedData });
				})
				.catch((err) => {
			...
			<>
				<InspectorControls>
					<div className="pg-setting-input-text">
						<PGtoggle title="Testimonials" initialOpen={true}>
							...
							<div>
								...
								<div className="flex items-center my-3 gap-2 flex-wrap">
									...
									<PGDropdown
										...
										options={sourceList}
										buttonTitle={
											sourceList[wrapper?.options?.source] != undefined
												? sourceList[wrapper?.options?.source].label
												: "Choose"
										}
										onChange={(option) => {

											var sourcePramsX = sourcePramsList[option.value]


											var options = { ...wrapper.options, source: option.value, sourcePrams: sourcePramsX };
										...
										{AIWriter && (
											<Popover position="bottom right">
												<div className="w-[800px] p-3">
													...
													<PGcssOpenaiPrompts value={""} formattedPrompt={formattedPrompt} promptsAgs={{ action: 'write', aiModel: 'gpt-4-turbo', objective: "generateTestimonials" }} autoUpdate={AIautoUpdate}
														...
														clickHandle={(value, action) => {
															...
															var items = [];
															var testimonialArr = JSON.parse(value);
															...
															if (action == 'replace') {
																...
																setAttributes({ testimonial: items });

															}

															if (action == 'prepend') {

																setAttributes({ testimonial: [...items, ...testimonial] });

															}

															if (action == 'append') {

																setAttributes({ testimonial: [...testimonial, ...items] });
							...
							{wrapper?.options?.source == 'google' && (
								<>
									<PanelRow>
										<label className="font-medium text-slate-900 ">{__("Meta Key", "combo-blocks")}</label>
										<InputControl
											value={wrapper.options.sourcePrams.metaKey}
											onChange={(newVal) => {
												var sourcePramsX = { ...wrapper.options.sourcePrams, metaKey: newVal };
												var options = { ...wrapper.options, sourcePrams: sourcePramsX };
							...
							{wrapper?.options?.source == 'trustpilot' && (
								<>
									<PanelRow>
										<label className="font-medium text-slate-900 ">{__("Meta Key", "combo-blocks")}</label>
										<InputControl
							...
							{wrapper?.options?.source?.length == 0 && (
								<>
													...
													title={
														<>
															<span
																...
																onClick={(ev) => {
																	var testimonialX = [...testimonial];
																	testimonialX.splice(index, 1);
																	setAttributes({
																		testimonial: testimonialX,
																	});
																}}>
																<Icon icon={close} />
															</span>
															<span className="handle cursor-pointer group bg-gray-600 hover:bg-gray-300 hover:text-white px-1 py-1">
																<Icon
																	icon={menu}
																	className="fill-white group-hover:fill-gray-700"
																/>
															</span>
															<span
																onClick={() => {
																	var testimonialX = [...testimonial];
																	var objToDup = { ...testimonialX[index] };
																	objToDup.id = Date.now();
																	testimonialX.splice(index + 1, 0, objToDup);
																	setAttributes({
																		testimonial: testimonialX,
																	});
																}}
												...
												<img src={item.url} alt="" />
													...
													<MediaUploadCheck>
														<MediaUpload
															...
															onSelect={(media) => {
																var testimonialX = [...testimonial];
																testimonialX[index].id = media.id;
																testimonialX[index].url = media.url;
																setAttributes({
																	testimonial: testimonialX,
																});
															}}
													...
													<div className="my-3">
														<label
															...
															{__("Name", "combo-blocks")}
														</label>
														<InputControl
															value={item.name}
															onChange={(newVal) => {
																var testimonialX = [...testimonial];
																testimonialX[index].name = newVal;
																setAttributes({
																	testimonial: testimonialX,
																});
															}}
													...
													<div className="my-3">
														...
														<div>
															<RichText
																...
																onChange={(content) => {
																	var testimonialX = [...testimonial];
																	testimonialX[index].message = content;
																	setAttributes({
																		testimonial: testimonialX,
																	});
																}}
													...
													<div className="my-3">
														...
														<InputControl
															...
															onChange={(newVal) => {
																var testimonialX = [...testimonial];
																testimonialX[index].website = newVal;
																setAttributes({
																	testimonial: testimonialX,
																});
															}}
													...
													<div className="my-3">
														...
														<InputControl
															...
															onChange={(newVal) => {
																var testimonialX = [...testimonial];
																testimonialX[index].role = newVal;
																setAttributes({
																	testimonial: testimonialX,
																});
															}}
													...
													<div className="my-3">
														...
														<div>
															<InputControl
																...
																onChange={(newVal) => {
																	var testimonialX = [...testimonial];
																	if (!testimonialX[index].categories) {
																		testimonialX[index].categories = [];
																	}
																	testimonialX[index].categories.push(newVal);
																	setAttributes({
																		testimonial: testimonialX,
																	});
																}}
													...
													<div className="flex flex-wrap gap-2 items-center">
														{testimonial[index].categories != undefined && (
															<>
																{testimonial[index]?.categories.map((item, j) => {
																	return (
																		<div className="flex  gap-2 items-center border border-solid pr-2">
																			<span
																				...
																				onClick={(ev) => {
																					var testimonialX = [...testimonial];
																					testimonialX[index]?.categories.splice(
																						j,
																						1
																					);
																					setAttributes({
										...
										return (
											<>
												<BlockContextProvider
													...
													{post.id ===
														(activeBlockContextId || testimonial[0]?.id) ? (
														<>
