...
registerBlockType(metadata, {
		...
		from: [
			{
				...
				transform: (attributes) => {
					return createBlock("combo-blocks/user-fields", {
						wrapper: {
							options: {
								class: "pg-author-fields",
								tag: "div",
							},
							styles: {
								display: {
		...
		to: [
			{
				...
				transform: (attributes) => {
					...
					if (metaKey !== "description") {
						// alert("Please select the proper user field!");
						return;
					}
					if (metaKey == "description") {
						return createBlock("core/post-author-biography", {});
					}
				},
			},
			{
				...
				transform: (attributes) => {
					var metaKey = attributes.metaKey;
					var field = attributes.field.options;
					if (metaKey !== "display_name") {
						// alert("Please select the proper user field!");
						return;
					}
					if (metaKey == "display_name") {
						return createBlock("core/post-author-name", {
							isLink: field.linkTo ? true : false,
			...
			},
			{
				...
				transform: (attributes) => {
					...
					if (metaKey == "avatar") {
						return createBlock("core/avatar", {
							isLink: field.linkTo ? true : false,
							linkTarget: field.linkTarget,
						});
					}
	...
	edit: function (props) {
		...
		var userFieldsBasic = {
			id: { label: __("ID", "combo-blocks"), value: "ID" },
			url: { label: __("URL", "combo-blocks"), value: "url" },
			registered: { label: __("Registered", "combo-blocks"), value: "registered" },
			display_name: {
				label: __("Display name", "combo-blocks"),
				value: "display_name",
			},
			first_name: { label: __("First name", "combo-blocks"), value: "first_name" },
			last_name: { label: __("Last name", "combo-blocks"), value: "last_name" },
			roles: { label: __("Roles", "combo-blocks"), value: "roles" },
			description: {
				label: __("Description", "combo-blocks"),
				value: "description",
			},
			avatar: { label: __("Avatar", "combo-blocks"), value: "avatar" },
			custom: {
				label: __("Custom Field", "combo-blocks"),
				value: "custom",
				isPro: true,
			},
			tutorInstructorCourseCount: {
				label: __("Tutor Instructor Course Count", "combo-blocks"),
				value: "tutorInstructorCourseCount",
			},
			tutorInstructorStudentCount: {
				label: __("Tutor Instructor Student Count", "combo-blocks"),
				value: "tutorInstructorStudentCount",
			},
			tutorEnrolledCourseCount: {
				label: __("Tutor  Enrolled Course Count", "combo-blocks"),
				value: "tutorEnrolledCourseCount",
			},
			tutorCompleteCourseCount: {
				label: __("Tutor Complete Course Count", "combo-blocks"),
				value: "tutorCompleteCourseCount",
			},
		};
		//let userFields = applyFilters("postAuthorFieldUserField", userFieldsBasic);
		let userFields = userFieldsBasic;

		var linkToArgsBasic = {
			noUrl: { label: __("No URL", "combo-blocks"), value: "noUrl" },
			postUrl: { label: __("Post URL", "combo-blocks"), value: "postUrl" },
			homeUrl: { label: __("Home URL", "combo-blocks"), value: "homeUrl" },
			userUrl: { label: __("User URL", "combo-blocks"), value: "userUrl" },
			userLink: { label: __("User Link", "combo-blocks"), value: "userLink" },
			userMail: {
				label: __("User Mail", "combo-blocks"),
				value: "userMail",
				isPro: true,
			},
			customField: {
		...
		function setLimitBy(option, index) {
			var options = { ...field.options, limitBy: option.value };
			setAttributes({ field: { ...field, options: options } });
		}
		return (
			<>
											...
											<>
												...
												<PanelRow>
													...
													<SelectControl
														...
														options={[
															{
																label: __("String", "combo-blocks"),
																value: "string",
															},
															{
																label: __("Image ID", "combo-blocks"),
																value: "imageId",
															},
														]}
														onChange={(newVal) => {
											...
											<div>
												<PanelRow>
													...
													<PGDropdown
														...
														// buttonTitle="Choose"
														buttonTitle={
															field.options?.limitBy == undefined
																? __("Choose", "combo-blocks")
																: limitByArgs[field.options?.limitBy] ==
																	undefined
																	? __("Choose", "combo-blocks")
																	: limitByArgs[field.options?.limitBy].label
														}
														onChange={setLimitBy}
														values={[]}></PGDropdown>
												</PanelRow>
												{(field.options?.limitBy == "word" ||
													field.options?.limitBy == "character") && (
														<PanelRow>
															<label
																htmlFor=""
																className="font-medium text-slate-900 ">
															...
															<InputControl
																value={field.options?.limitCount}
																type="number"
																onChange={(newVal) => {
																	var options = {
																		...field.options,
											...
											<div>
												<div className="flex items-center justify-between gap-2">
													<label
														htmlFor=""
														className="font-medium text-slate-900">
														{__("Avatar Size", "combo-blocks")}
													</label>
													<InputControl
														value={field.options.avatarSize}
														onChange={(newVal) => {
															var options = {
																...field.options,
																avatarSize: newVal,
												...
												<PanelRow>
													...
													<SelectControl
														label=""
														value={field.options?.defaultAvatar}
														options={[
															{ label: "Select..", value: "" },
															{ label: "404", value: "404" },
															{ label: "Retro", value: "retro" },
															{ label: "Robohash", value: "robohash" },
												...
												<PanelRow>
													...
													<SelectControl
														label=""
														value={field.options?.avatarRating}
														options={[
															{ label: "Select..", value: "" },
															{ label: "G", value: "G" },
															{ label: "PG", value: "PG" },
