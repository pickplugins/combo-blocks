		...
		from: [
			{
				...
				transform: (attributes) => {
					...
					if (attributes.text !== undefined) {
						let result = attributes.text.includes("[");
						if (result) {
							var shortcodeStr = attributes.text.replace("[", "");
							shortcodeStr = shortcodeStr.replace("]", "");
							var shortcodeArr = shortcodeStr.split(" ");
							var shortcodeKey = shortcodeArr[0];
							keyX = shortcodeKey; // Update keyX with shortcodeKey
							shortcodeArr.shift();
							shortcodeArr.map((x) => {
								var shortcodePrams = {};
								var attrArr = x.split("=");
								...
								shortcodePrams.val =
									attrArr[1] == undefined ? "" : attrArr[1].replaceAll('"', "");
								pramsX.push(shortcodePrams); // Update pramsX with shortcodePrams
							});
						}
					}
					return createBlock("combo-blocks/shortcode", {
						shortcode: {
							options: {
								key: keyX,
								id: "",
		...
		to: [
			{
				...
				transform: (attributes) => {
					var shortcodeX = "";
					if (
						attributes.shortcode.options.key !== undefined ||
						attributes.shortcode.options.key !== ""
					) {
						var shortcodeX = "[" + attributes.shortcode.options.key;
						if (
							attributes.shortcode.options.prams &&
							Array.isArray(attributes.shortcode.options.prams)
						) {
							shortcodeX +=
								" " +
								attributes.shortcode.options.prams
									.filter(
										(param) =>
											param.id !== "" && param.label !== "" && param.val !== ""
									)
									.map((x) => `${x.id}="${x.val}"`)
									.join(" ");
						}
				...
				<InspectorControls>
					<div className="pg-setting-input-text">
						<PGtoggle
							...
							<p>
								{__(
									"You can paste the shortcode, please use following format when pasting",
									"combo-blocks"
								)}
							</p>
