...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var itemSelector = blockClass + " .pg-testimonial-showcase-item";
		...
		useEffect(() => {
			...
			nthItemStyle.map((x, i) => {
				var selector = `${blockClass} .pg-testimonial-showcase-item:nth-child(${i + 1
					})`;
		...
		const addChild = () => {
			...
			const slide = createBlock("combo-blocks/team-members");
		...
		function onChangeStyleNthItem(sudoScource, newVal, attr, obj, extra) {
			...
			var selector = `${blockClass} .pg-testimonial-showcase-item:nth-child(${index + 1
				})`;
		...
		function onBulkAddNthItem(sudoScource, cssObj, extra) {
			...
			setAttributes({ nthItemStyle: nthItemStyleX });
			var selectorX = `${blockClass} .pg-testimonial-showcase-item:nth-child(${index + 1
				})`;
		...
		function onResetNthItem(sudoSources, extra) {
			...
			if (nthObj) {
				...
				Object.entries(sudoSources).forEach(([sudoSource]) => {
					if (obj[sudoSource]) {
						obj[sudoSource] = {};
						const selector = `${blockClass} .pg-testimonial-showcase-item:nth-child(${index + 1
							})`;
	...
	save: function (props) {
		...
		const blockProps = useBlockProps.save({
			className: ` ${blockId} pg-testimonial-showcase-item`,
		});
