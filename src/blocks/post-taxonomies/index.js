...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		function findTaxById(id) {
			return postObjects.find((item) => item.id === id);
		}
		const taxNameX = findTaxById(taxonomies.options.taxName);
		useEffect(() => {
		...
		var postTaxonomiesIconPositionBasic = {
			...
			beforeItem: {
				label: __("Before Each Item", "combo-blocks"),
				value: "beforeItem",
				isPro: true,
			},
			afterItem: {
				label: __("After Each Item", "combo-blocks"),
				value: "afterItem",
				isPro: true,
			},
		};
		let iconPositionArgs = applyFilters(
			"comboBlocksPostTaxonomiesIconPosition",
			postTaxonomiesIconPositionBasic
		);
		function setIconPosition(option, index) {
			var options = { ...icon.options, position: option.value };
			setAttributes({ icon: { ...icon, options: options } });
		}
		var postTaxonomiesLinkToBasic = {
		...
		let linkToArgs = applyFilters(
			"comboBlocksPostTaxonomiesLinkTo",
			postTaxonomiesLinkToBasic
		);
		...
		useEffect(() => {
			...
			if (iconSrc.length == 0) {
				...
				iconHtml = `<span class="${iconSrc} ${icon.options.class}"></span>`;
			}
		...
		function onPickBlockPatterns(content, action) {
			...
			if (action == "applyStyle") {
				...
				if (taxonomiesX != undefined) {
					...
					blockCssObj[itemSelector] = taxonomiesY;
				}
		...
		function onResetFrontText(sudoSources) {
			let obj = Object.assign({}, frontText);
			Object.entries(sudoSources).map((args) => {
				var sudoScource = args[0];
				if (obj[sudoScource] == undefined) {
		...
		function onResetTermTitle(sudoSources) {
			let obj = Object.assign({}, termTitle);
			Object.entries(sudoSources).map((args) => {
				var sudoScource = args[0];
				if (obj[sudoScource] == undefined) {
		...
		function onResetPostCount(sudoSources) {
			let obj = Object.assign({}, postCount);
			Object.entries(sudoSources).map((args) => {
				var sudoScource = args[0];
				if (obj[sudoScource] == undefined) {
						...
						<div className="px-3 my-4">
							<PanelRow>
								...
								<PGDropdown
									...
									// buttonTitle="Choose"
									buttonTitle={
										taxNameX?.label == undefined || taxNameX?.label.length == 0
							...
							<PanelRow>
								<label htmlFor="" className="font-medium text-slate-900 ">
									{__("Custom Taxonomy", "combo-blocks")}
								</label>
								<InputControl
