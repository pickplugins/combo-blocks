...
registerBlockType(metadata, {
	...
	edit: function (props) {
		...
		var parentIcon =
			context["combo-blocks/icon"] == undefined ? [] : context["combo-blocks/icon"];
		var parentItem =
			context["combo-blocks/listItemTag"] == undefined
				? []
				: context["combo-blocks/listItemTag"];
		var parentWrapperTag = parentItem.options.tag;
		if (parentWrapperTag !== undefined) {
			useEffect(() => {
				var options = { ...wrapper.options, tag: parentWrapperTag };
				setAttributes({
		...
		const CustomTagWrapper =
			wrapper.options.tag?.length != 0 ? `${wrapper.options.tag}` : "li";
		// useEffect(() => {
		// 	var optionX = {...wrapper.options, tag: parentItem.options.tag}
		// 	setAttributes({wrapper: optionX});
		// }, [parentItem]);
		useEffect(() => {
			var blockCssObj = {};
		...
		const MY_TEMPLATE = [
			[
				...
				{
					text: {
						options: { content: "Hello World!", tag: "span", class: "pg-text" },
					},
				},
			],
		];
