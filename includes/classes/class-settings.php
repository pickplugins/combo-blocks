<?php
/*
* @Author 		pickplugins
* Copyright: 	2015 pickplugins.com
*/
if (!defined('ABSPATH')) exit;  // if direct access
class class_combo_blocks_settings
{
    public function __construct()
    {
        add_action('admin_menu', array($this, 'combo_blocks_menu_init'));
        //add_action('admin_menu', array($this, '_access_non_admin'));
    }
    public function _access_non_admin()
    {
        if (!current_user_can('administrator')) :
            remove_menu_page('edit.php?post_type=combo_blocks');
            remove_menu_page('post-new.php?post_type=combo_blocks');
            remove_menu_page('edit.php?post_type=combo_blocks_templates');
            remove_menu_page('edit.php?post_type=combo_blocks_layout');
        endif;
    }
    public function combo_blocks_menu_init()
    {
        if (!current_user_can('administrator')) return;
        $combo_blocks_info = get_option('combo_blocks_info');

        add_menu_page(__('Combo Blocks', 'combo-blocks'), __('Combo Blocks', 'combo-blocks'), 'manage_options', 'combo-blocks', array($this, 'combo_blocks'), 'data:image/svg+xml;base64,', 90);
        //add_submenu_page('combo-blocks', __('Create New', 'combo-blocks'), __('Create New', 'combo-blocks'), 'manage_options', 'post-new.php?post_type=combo_blocks', '', 20);
        // add_submenu_page('combo-blocks', __('Import Layouts', 'combo-blocks'), __('Import Layouts', 'combo-blocks'), 'manage_options', 'import_layouts', array($this, 'import_layouts'), 70);
        //add_submenu_page('combo-blocks', __('Layouts', 'combo-blocks'), __('All Layouts', 'combo-blocks'), 'manage_options', 'edit.php?post_type=combo_blocks_layout',);
        //add_submenu_page('combo-blocks', __('Settings', 'combo-blocks'), __('Settings', 'combo-blocks'), 'manage_options', 'combo-blocks-settings', array($this, 'settings'), 80);
        //add_submenu_page('combo-blocks', __('Dashboard', 'combo-blocks'), __('Dashboard', 'combo-blocks'), 'manage_options', 'combo-blocks-dashboard', array($this, 'dashboard'), 80);
        add_submenu_page('combo-blocks', __('Overview', 'combo-blocks'), __('Overview', 'combo-blocks'), 'manage_options', 'combo-blocks-overview', array($this, 'overview'), 80);
    }
    public function overview()
    {
        include(combo_blocks_dir . 'includes/menu/overview.php');
    }
    public function dashboard()
    {
        include(combo_blocks_dir . 'includes/menu/dashboard.php');
    }
    public function combo_blocks()
    {
        include(combo_blocks_dir . 'includes/menu/dashboard.php');
    }


    public function settings()
    {
        include(combo_blocks_dir . 'includes/menu/settings.php');
    }

    public function import_layouts()
    {
        include(combo_blocks_dir . 'includes/menu/import-layouts.php');
    }
}
new class_combo_blocks_settings();
